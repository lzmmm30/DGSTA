...
class TrafficStateDataset(AbstractDataset):
    ...
    def __init__(self, config):
        ...
        self.data = None
        self.feature_name = {'X': 'float', 'y': 'float', 'ind': 'int'}
        self.adj_mx = None
        self.scaler = None
    ...
    def _load_rel(self):
        ...
        if self.calculate_weight_adj:  # false
    ...
    # 对交通流数据进行时间嵌入  (17836,170,9)
    def _add_external_information_3d(self, df, ext_data=None):
        num_samples, num_nodes, feature_dim = df.shape  # 17836,170,1
        ...
        if self.add_time_in_day and not is_time_nan:
            ...
            data_list.append(time_in_day)  # (17836,170,2) (traffic_flow,time)
        # 时间嵌入，每周
        if self.add_day_in_week and not is_time_nan:
            ...
            for day in self.timesolts.astype("datetime64[D]"):
                ...
                # %Y-%m-%d  2016-07-01  weekday()输出0-6，代表周一-周日
            day_in_week = np.zeros(shape=(num_samples, num_nodes, 7))  # (17836,170,7)
            day_in_week[np.arange(num_samples), :, dayofweek] = 1
            # 给每个时间点的170个节点赋周特征，最后一维长度为7，是周几就将该位置赋为0-6
            data_list.append(day_in_week)  # (17836,170,9)
            ...
            if not is_time_nan:
                ...
                if ext_data.shape[0] == df.shape[0]:
                    ...
                    for i in range(select_data.shape[1]):
                        ...
                        data_ind = np.tile(data_ind, [1, num_nodes, 1]).transpose((2, 1, 0))
                        data_list.append(data_ind)
        data = np.concatenate(data_list, axis=-1)  # (17836,170,9)
        return data
    ...
    # 对数据集进行划分，得到x和y (17836,12,170,9)
    def _generate_input_data(self, df):
        num_samples = df.shape[0]  # 17836
        x_offsets = np.sort(np.concatenate((np.arange(-self.input_window + 1, 1, 1),)))  # -11~0
        y_offsets = np.sort(np.arange(1, self.output_window + 1, 1))  # 1~12

        x, y = [], []
        min_t = abs(min(x_offsets))  # 11
        max_t = abs(num_samples - abs(max(y_offsets)))  # 17836-12
        for t in range(min_t, max_t):
            ...
            y.append(y_t)
        x = np.stack(x, axis=0)
        y = np.stack(y, axis=0)  # (17836,12,170,9)
        return x, y

    # 得到处理好的训练数据 x和y (17836,12,170,9)
    def _generate_data(self):
        ...
        for filename in data_files:
            ...
            if self.load_external:
                df = self._add_external_information(df, ext_data)  # 返回(17836,170,9)
            x, y = self._generate_input_data(df)  # (17836,12,170,9)
            x_list.append(x)
            y_list.append(y)
    ...
    # 分割得到训练集、测试集、验证集 (17836,12,170,9)
    def _split_train_val_test(self, x, y):
        test_rate = 1 - self.train_rate - self.eval_rate
        # train_rate 0.6 eval_rate 0.2 test_rate 0.2
        ...
        x_train, y_train, ind_train = x[int(num_train*(1 - self.part_train_rate)):num_train], y[int(num_train*(1 - self.part_train_rate)):num_train], ind[int(num_train*(1 - self.part_train_rate)):num_train]
        x_val, y_val, ind_val = x[num_train: num_train + num_val], y[num_train: num_train + num_val], ind[num_train: num_train + num_val]
        x_test, y_test, ind_test = x[-num_test:], y[-num_test:], ind[-num_test:]
        self._logger.info("train\t" + "x: " + str(x_train.shape) + ", y: " + str(y_train.shape) + ", ind: " + str(ind_train.shape))
        self._logger.info("eval\t" + "x: " + str(x_val.shape) + ", y: " + str(y_val.shape) + ", ind: " + str(ind_val.shape))
        self._logger.info("test\t" + "x: " + str(x_test.shape) + ", y: " + str(y_test.shape) + ", ind: " + str(ind_test.shape))
        ...
        return x_train, y_train, ind_train, x_val, y_val, ind_val, x_test, y_test, ind_test

    # 返回训练集、测试集、验证集 (17836,12,170,9)
    def _generate_train_val_test(self):
        x, y = self._generate_data()  # (17836,12,170,9)
    ...
    def _load_cache_train_val_test(self):
        ...
        ind_train = cat_data['ind_train']
        x_test = cat_data['x_test']
        y_test = cat_data['y_test']
        ind_test = cat_data['ind_test']
        x_val = cat_data['x_val']
        y_val = cat_data['y_val']
        ind_val = cat_data['ind_val']
        self._logger.info(
            "train\t" + "x: " + str(x_train.shape) + ", y: " + str(y_train.shape) + ", ind: " + str(ind_train.shape))
        self._logger.info(
            "eval\t" + "x: " + str(x_val.shape) + ", y: " + str(y_val.shape) + ", ind: " + str(ind_val.shape))
        self._logger.info(
            "test\t" + "x: " + str(x_test.shape) + ", y: " + str(y_test.shape) + ", ind: " + str(ind_test.shape))
        return x_train, y_train, ind_train, x_val, y_val, ind_val, x_test, y_test, ind_test
